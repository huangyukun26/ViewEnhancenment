...
class Focal_loss(nn.Module):
    def __init__(self, alpha=0.25, gamma=4, num_classes=3, size_average=True):
...
def test_single_volume_windowview(image, label, net, classes, multimask_output, patch_size=[256, 256],
    ...
    if test_save_path is not None:
        prd_itk = Image.fromarray((train_id_to_color_7[prediction]).astype(np.uint8))


        prd_itk.save(test_save_path + '/' + case.split("/")[-1][:-4] + "_pred.png")
