...
def trainer_windowview(args, model, snapshot_path, multimask_output, low_res):
    from evaluator import Evaluator
    from datasets.dataset_windowview import Windowview_dataset, RandomGenerator, transpose_self
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    batch_size = args.batch_size * args.n_gpu
    # max_iterations = args.max_iterations
    db_train = Windowview_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size], low_res=[low_res, low_res])]))
    print("The length of train set is: {}".format(len(db_train)))

    db_val = Windowview_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="val",transform=transforms.Compose([transpose_self(output_size=[args.img_size, args.img_size], low_res=[low_res, low_res])]))
