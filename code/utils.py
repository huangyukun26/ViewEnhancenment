...
train_id_to_color_7=np.asarray([[255,255,0],[0,0,255],[0,255,0],[170,170,170],[96,25,134],[253,167,206],[255,0,0]])
train_id_to_color_4=np.asarray([[255,255,0],[0,0,255],[0,255,0],[96,25,134],])
...
def test_single_volume_landfill(image, label, net, classes, multimask_output, patch_size=[256, 256],
                       test_save_path=None, case=None, evalutator=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
            prediction = np.zeros_like(label)
        # for ind in range(image.shape[0]):
    ...
    evalutator.add_batch(label, prediction)

    

    if test_save_path is not None:
        img_itk = Image.fromarray((image*255).astype(np.uint8))
        prd_itk = Image.fromarray((prediction*255).astype(np.uint8))
        lab_itk = Image.fromarray((label*255).astype(np.uint8))

        img_itk.save(test_save_path + '/' + case[:-4] + "_img.png")
        prd_itk.save(test_save_path + '/' + case[:-4] + "_pred.png")
        lab_itk.save(test_save_path + '/' + case[:-4] + "_gt.png")
    return evalutator


def test_single_volume_windowview(image, label, net, classes, multimask_output, patch_size=[256, 256],
                       test_save_path=None, case=None, evalutator=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
            prediction = np.zeros_like(label)
    ...
    if test_save_path is not None:
        ...
        prd_itk = Image.fromarray((train_id_to_color_4[prediction]).astype(np.uint8))
        lab_itk = Image.fromarray((train_id_to_color_4[label]).astype(np.uint8))

        img_itk.save(test_save_path + '/' + case[:-5] + "_img.png")
        prd_itk.save(test_save_path + '/' + case[:-5] + "_pred.png")
        lab_itk.save(test_save_path + '/' + case[:-5] + "_gt.png")
    return evalutator
